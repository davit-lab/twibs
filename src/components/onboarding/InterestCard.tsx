...
export default function InterestCard({
  ...
  const designColor = 'hsl(var(--primary))';
  const resolvedColor = color || designColor;
    ...
    <button
      ...
      style={
        selected
          ? ({
              '--cat': designColor,
              background: `linear-gradient(160deg, ${designColor} 0%, hsl(var(--primary) / 0.9) 100%)`,
              boxShadow: '0 14px 34px -12px hsl(var(--primary) / 0.7), inset 0 0 0 1px rgba(255,255,255,0.22)',
            } as React.CSSProperties)
          : ({ '--cat': designColor } as React.CSSProperties)
      }
      className={cn(
    ...
    >
      {selected && (
        <span className="absolute top-2 right-2 grid place-items-center w-5 h-5 rounded-full bg-white/95 shadow-sm">
          <Check className="w-3 h-3" style={{ color: designColor }} strokeWidth={3.5} />
        </span>
      )}
      ...
      <span
        ...
        style={{
          backgroundColor: selected ? 'rgba(255,255,255,0.24)' : 'hsl(var(--primary) / 0.08)',
        }}
      >
        <Icon
          className={cn('w-6 h-6 sm:w-7 sm:h-7 transition-transform duration-200', selected && 'scale-110')}
          style={{ color: selected ? '#ffffff' : designColor }}
