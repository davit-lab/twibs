-- ============================================================
-- Groups: Facebook-style communities with posts & answers
-- ============================================================
...
ALTER TABLE public.groups
  DROP CONSTRAINT IF EXISTS groups_privacy_check;
ALTER TABLE public.groups
  ADD CONSTRAINT groups_privacy_check CHECK (privacy IN ('public', 'private'));
...
DROP TRIGGER IF EXISTS update_group_post_count_trigger ON public.group_posts;
...
DROP TRIGGER IF EXISTS update_group_post_like_count_trigger ON public.group_post_likes;
...
DROP TRIGGER IF EXISTS update_group_post_comment_count_trigger ON public.group_post_comments;
...
-- RLS
-- ============================================================
-- Ensure objects are owned by postgres so SECURITY DEFINER RPCs
-- bypass RLS (idempotent even if tables pre-existed).
...
ALTER FUNCTION public.join_group OWNER TO postgres;
ALTER FUNCTION public.leave_group OWNER TO postgres;
ALTER FUNCTION public.is_group_member OWNER TO postgres;
ALTER FUNCTION public.is_group_owner_or_admin OWNER TO postgres;
...
-- group_posts
DROP POLICY IF EXISTS "Group posts visible to members or in public groups" ON public.group_posts;
...
DROP POLICY IF EXISTS "Authors or group admins can update group posts" ON public.group_posts;
...
DROP POLICY IF EXISTS "Users can update their own group comments" ON public.group_post_comments;
